						...
						<table class="table table-hover">
							<thead>
								<tr>
									...
									<th>Description</th>
									<th>Image</th>
									<th>Created for</th> <!-- Add a new column for User -->
									<th>Actions</th>
								</tr>
							</thead>
							<tbody>
								...
								<tr>
									...
									<td>
										@if ($userActivity->image)
										<img src="{{ asset('storage/' . $userActivity->image) }}" style="width: 100px; height: auto;">
										@else
										No image
										@endif
									</td>
									<td>{{ $userActivity->user->name ?? 'N/A' }}</td> <!-- Display the username -->
									<td>
										<form action="{{ route('activities.destroy', $userActivity->id) }}" method="POST" style="display: inline;" onsubmit="return confirm('Are you sure you want to delete this activity?');">
											@csrf
											@method('DELETE')
											<button type="submit" class="btn btn-danger">Delete</button>
										</form>
										<a href="{{ route('activities.edit', $userActivity->id) }}" class="btn btn-warning">Edit</a>
									</td>
								</tr>
								@endforeach
							</tbody>
